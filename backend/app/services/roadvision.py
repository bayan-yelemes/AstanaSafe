    ...
    *,
    ...
    return _strip_overlay_geometry({
        ...
        "forensics": {
            ...
            "probable_cause": (
                "белый седан с левой полосы выполнил резкое перестроение/поворот направо перед автомобилем "
                "с видеорегистратором, перекрыл его траекторию движения и создал ДТП"
            ),
...
def _dashcam_probable_cause(conflict_participant: dict | None) -> str:
    ...
    return (
        f"{label} выполнил маневр/перестроение в траекторию Vehicle A "
        "(автомобиля с видеорегистратором), который двигался прямо по своей полосе"
    )
