...
const ROADVISION_KZ_TEXT = new Map([
  ["Требуется проверка", "Тексеру қажет"],
  ["Не назначен", "Тағайындалмаған"],
  ["Vehicle A (регистратор)", "Vehicle A (тіркеуші)"],
  ["Vehicle B (белый седан впереди)", "Vehicle B (алдындағы ақ седан)"],
  ["автомобиль с видеорегистратором", "видеотіркеуіші бар автокөлік"],
  ["попутный маневрирующий автомобиль", "бір бағытта маневр жасайтын автокөлік"],
  ["движется прямо по правой полосе, когда белый седан с левой полосы смещается направо перед капотом", "оң жақ жолақпен тура қозғалып келеді, сол жақ жолақтағы ақ седан капоттың алдында оңға ауысады"],
  ["плавное сближение, затем резкое торможение из-за перестроения белого седана справа перед регистратором", "біртіндеп жақындау, кейін тіркеушінің алдында ақ седанның оңға ауысуына байланысты күрт тежеу"],
  ["движется с левой полосы и резко перестраивается/поворачивает направо перед Vehicle A", "сол жақ жолақтан қозғалып, Vehicle A алдында оңға қарай күрт ауысады/бұрылады"],
  ["резкое боковое смещение вправо без безопасного интервала перед контактом", "жанасуға дейін қауіпсіз аралық сақтамай оңға қарай күрт бүйірлік ауысу"],
  ["признаки опасного резкого перестроения/поворота направо перед автомобилем с регистратором", "тіркеуші автокөліктің алдында оңға қарай қауіпті күрт ауысу/бұрылу белгілері"],
  ["Начало сближения", "Жақындасудың басталуы"],
  ["Vehicle A движется за белым седаном Vehicle B; расстояние между ними постепенно сокращается.", "Vehicle A ақ седан Vehicle B артынан қозғалады; олардың арақашықтығы біртіндеп қысқарады."],
  ["белый седан находится непосредственно перед капотом регистратора", "ақ седан тіркеуші капотының дәл алдында орналасқан"],
  ["Белый седан готовится к смещению", "Ақ седан ауысуға дайындалып жатыр"],
  ["Vehicle B находится перед регистратором левее траектории Vehicle A и начинает менять положение относительно полосы.", "Vehicle B тіркеушінің алдында, Vehicle A траекториясының сол жағында орналасқан және жолаққа қатысты орнын өзгерте бастайды."],
  ["белый седан впереди расположен левее траектории регистратора", "алдағы ақ седан тіркеуші траекториясының сол жағында орналасқан"],
  ["Резкий поворот направо", "Оңға күрт бұрылу"],
  ["Когда Vehicle A подъезжает вперед, белый седан Vehicle B резко смещается с левой полосы направо и перекрывает путь регистратору.", "Vehicle A алға жақындағанда, ақ седан Vehicle B сол жақ жолақтан оңға қарай күрт ауысып, тіркеушінің жолын бөгейді."],
  ["Vehicle B оказывается под углом перед капотом и входит в правую траекторию Vehicle A", "Vehicle B капоттың алдында бұрышпен орналасып, Vehicle A оң жақ траекториясына кіреді"],
  ["Момент ДТП", "ЖКО сәті"],
  ["Vehicle B пересекает траекторию Vehicle A перед капотом, происходит контакт с автомобилем-регистратором.", "Vehicle B капоттың алдында Vehicle A траекториясын кесіп өтеді, тіркеуші автокөлікпен жанасу болады."],
  ["белый седан занимает переднюю часть кадра вплотную к капоту регистратора", "ақ седан кадрдың алдыңғы бөлігін тіркеуші капотына өте жақын алып тұр"],
  ["Последствия маневра", "Маневр салдары"],
  ["После контакта Vehicle B уходит правее/вперед из зоны перед капотом, Vehicle A продолжает движение с малой скоростью.", "Жанасудан кейін Vehicle B капот алдындағы аймақтан оңға/алға шығады, Vehicle A төмен жылдамдықпен қозғалысын жалғастырады."],
  ["белый седан смещен впереди от регистратора, впереди формируется кратковременное замедление", "ақ седан тіркеушіден алға ығысқан, алда қысқа мерзімді баяулау қалыптасады"],
  ["Отчет подготовлен по ключевым кадрам именно для видео 1000081819.mp4.", "Есеп 1000081819.mp4 бейнесінің негізгі кадрлары бойынша дайындалды."],
  ["Событие описано как ДТП: белый седан с левой полосы резко уходит направо перед автомобилем с регистратором.", "Оқиға ЖКО ретінде сипатталды: ақ седан сол жақ жолақтан тіркеуші автокөліктің алдына оңға қарай күрт ауысады."],
  ["Госномера в ролике не читаются надежно и оставлены на ручную проверку.", "Бейнедегі мемлекеттік нөмірлер сенімді оқылмайды және қолмен тексеруге қалдырылды."],
  ["Хронология построена по выбранному сценарию и требует проверки оператором.", "Хронология таңдалған сценарий бойынша құрылды және оператордың тексеруін қажет етеді."],
  ["Госномер не удалось надежно извлечь в demo-режиме; требуется ручное подтверждение.", "Demo режимінде мемлекеттік нөмірді сенімді анықтау мүмкін болмады; қолмен растау қажет."],
  ["Госномер не удалось надежно извлечь в MVP-режиме; требуется ручное подтверждение.", "MVP режимінде мемлекеттік нөмірді сенімді анықтау мүмкін болмады; қолмен растау қажет."],
  ["передняя зона автомобиля с регистратором и боковая/задняя часть белого седана", "тіркеуші автокөліктің алдыңғы аймағы және ақ седанның бүйір/артқы бөлігі"],
  ["Предварительно: белый седан, двигаясь с левой полосы, резко перестроился направо перед автомобилем с видеорегистратором, перекрыл его траекторию и спровоцировал столкновение", "Алдын ала: ақ седан сол жақ жолақтан қозғалып келе жатып, видеотіркеуіші бар автокөліктің алдына оңға қарай күрт ауысып, оның траекториясын жауып, соқтығысуды тудырды"],
  ["признаки резкого перестроения/поворота направо без безопасного интервала перед Vehicle A", "Vehicle A алдында қауіпсіз аралық сақтамай оңға қарай күрт ауысу/бұрылу белгілері"],
  ["00:04-00:06: белый седан находится левее траектории регистратора и начинает смещение", "00:04-00:06: ақ седан тіркеуші траекториясының сол жағында орналасып, ауысуды бастайды"],
  ["00:06.5-00:07: Vehicle B резко уходит направо в зону движения Vehicle A", "00:06.5-00:07: Vehicle B Vehicle A қозғалыс аймағына оңға қарай күрт кіреді"],
  ["00:07: белый седан находится вплотную перед капотом регистратора, фиксируется момент ДТП", "00:07: ақ седан тіркеуші капотының дәл алдында, ЖКО сәті тіркеледі"],
  ["AI формирует предварительное аналитическое заключение. Юридическая виновность устанавливается только уполномоченным органом.", "AI алдын ала аналитикалық қорытынды қалыптастырады. Заңды кінәлілікті тек уәкілетті орган анықтайды."],
  ["кратковременная блокировка правой полосы", "оң жақ жолақтың қысқа мерзімді бөгелуі"],
  ["10 мин", "10 мин"],
  ["правая полоса движения", "оң жақ қозғалыс жолағы"],
  ["Проверить фрагмент 00:04-00:07, где белый седан с левой полосы резко уходит направо перед регистратором.", "Ақ седан сол жақ жолақтан тіркеушінің алдына оңға қарай күрт ауысатын 00:04-00:07 үзіндісін тексеру."],
  ["Отметить Vehicle B как участника с признаками нарушения при маневре и сохранить фрагмент момента ДТП.", "Vehicle B-ні маневр кезінде бұзушылық белгілері бар қатысушы ретінде белгілеу және ЖКО сәтінің үзіндісін сақтау."],
  ["При необходимости уточнить госномера по исходному видео вручную.", "Қажет болса, мемлекеттік нөмірлерді бастапқы бейне бойынша қолмен нақтылау."],
]);
...
function translateRoadVisionText(value, language, copy = ROADVISION_COPY.en) {
  ...
  if (!["en", "kz"].includes(normalizedLanguage)) return value;

  const directTranslation =
    normalizedLanguage === "kz"
      ? ROADVISION_KZ_TEXT.get(value)
      : ROADVISION_EN_TEXT.get(value);
  if (directTranslation) return directTranslation;
  if (normalizedLanguage === "kz") return value;
